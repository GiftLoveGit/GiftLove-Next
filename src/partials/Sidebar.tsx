...
import { FaCreditCard, FaDesktop, FaDoorClosed } from "react-icons/fa"
...
function Sidebar({ currentPage }: SidebarProps) {
  ...
  return (
    <div className="mt-20" style={{ marginTop: '150px' }}>
      ...
      <div className="d-flex justify-content-between align-items-center mb-3 px-2 px-sm-5 mt-4 text-light">
        <h4 className="fw-bold">{currentPage}</h4>
        <p className="d-none d-sm-flex fw-bold">
          <PersonCircle className="fs-4 me-2 d-sm-flex d-none" /> Nome
          do user
        </p>
        ...
        <div className="offcanvas-body">
          <ul className="nav flex-column sidebar">
            ...
            <li className="nav-item py-2">
              <Link
                href="/#inicio"
                className={`nav-link gl-gray rounded-3 ${pathname === "#inicio" ? "active" : ""}`}
              >
                <FaDesktop /> Site
              </Link>
            </li>
            ...
            <li className="nav-item py-2">
              <a className="nav-link gl-gray rounded-3" href="#" onClick={handleSubmit}>
